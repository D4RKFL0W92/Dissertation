...
static uint8_t printMmapFlags(int flags)
{
  ...
  {
    strcat(flagBuff, MAP_ANONYMOUS_STR);
    flagsSet += 1;
  }
  printf("%s", flagBuff);
...
static void printNamespaceType(int fd, int namespaceType)
{
  ...
  {
  /*
   * Some versions of Unix will not support certain namespaces,
   * this is why literal values are used for switch statement.
   */
...
static int print_kcmpType(int type)
{
  ...
  case KCMP_EPOLL_TFD:
    printf("KCMP_EPOLL_TFD");
    return KCMP_EPOLL_TFD;
...
static int8_t printIoVectorData64(int executablePID, struct iovec * vec, int vecCnt)
{
  void * iovecData = NULL;
  int8_t err = ERR_NONE;
  ...
  {
    struct iovec vecElement = {0};

    memcpy(&vecElement, vec + i * sizeof(struct iovec *), sizeof(struct iovec));

    iovecData = (struct iovec *) malloc(vecElement.iov_len);

    // Read in the data described by the iovec struct and print the bytes.
    if(vecElement.iov_base > 0 && vecElement.iov_len > 0)
    {
      err = readProcessMemoryFromPID(executablePID,
                                    vecElement.iov_base,
                                    iovecData,
                                    vecElement.iov_len);
      if(err != ERR_NONE)
      ...
      printf("IOVEC Data %d\n", i + 1);
      dumpHexBytesFromOffset(iovecData, 0, vecElement.iov_len);
      free(iovecData);
    }
  }
}
...
/***********************************************************************************/
  case SYS_stat:
    ...
    printf("stat(filename=\"%s\", stat-buff-addr=%s)\n",
                 executableHandle->regs.rdi,
                 executableHandle->regs.rsi);
...
/***********************************************************************************/
  case SYS_ioctl:
    // TODO: Print the cmd and arg in human readable format.
    printf("ioctl(fd=%d, cmd=0x%08x, arg=%ld)\n",
                  executableHandle->regs.rdi,
                  executableHandle->regs.rsi,
                  executableHandle->regs.rdx);
...
/***********************************************************************************/
  case SYS_pread64:
    ...
    {
      printf("pread64(fd=%d, buff-add=%p, count=0x%08x, offset=%p)\n",
                      executableHandle->regs.rdi,
                      executableHandle->regs.rsi,
                      executableHandle->regs.rdx,
                      executableHandle->regs.r10);
...
/***********************************************************************************/
  case SYS_pwrite64:
    tmpBuffer1 = malloc(executableHandle->regs.rdx * sizeof(struct iovec *));
    if (tmpBuffer1 == NULL)
    {
      return ERR_MEMORY_ALLOCATION_FAILED;
    }
    ...
    err = readProcessMemoryFromPID(executableHandle->pid,
                                   executableHandle->regs.rsi,
                                   tmpBuffer1,
                                   executableHandle->regs.rdx * sizeof(struct iovec *));
    if(err != ERR_NONE)
    {
      return err;
    }
...
/***********************************************************************************/
  case SYS_getrlimit:
    ...
    printf("getrlimit(resource=0x%08x, rlimit_cur=%d, rlimit-max=%d)\n",
                      executableHandle->regs.rdi,
                      resLimit.rlim_cur,
                      resLimit.rlim_max);
...
/***********************************************************************************/
  case SYS_fstatfs:
    ...
    printf("fstatfs(fd=%d)\n", executableHandle->regs.rdi);
    // TODO: Print the data from statfs struct.
...
/***********************************************************************************/
  case SYS_sched_rr_get_interval:
    ...
    err = readProcessMemoryFromPID(executableHandle->pid, &timeSpec, executableHandle->regs.rsi, sizeof(struct timespec));
    if (err != ERR_NONE)
    {
      return err;
    }

    printf("sched_rr_get_interval(pid=%d, seconds=%d.%d)\n",
                                  executableHandle->regs.rdi,
                                  timeSpec.tv_sec,
                                  timeSpec.tv_nsec);
...
/***********************************************************************************/
  case SYS_arch_prctl:
    printf("arch_prctl(task_struct-addr=%p, code=%d, addr=%p)\n",
                       executableHandle->regs.rdi,
                       executableHandle->regs.rsi,
                       executableHandle->regs.rdx);
...
/***********************************************************************************/
  case SYS_settimeofday:
    ...
    printf("settimeofday(seconds=%d.%d)\n", tm.tv_sec, tm.tv_usec);
...
/***********************************************************************************/
  case SYS_mount:
    ...
    err = readStringFromProcessMemory(executableHandle->pid, executableHandle->regs.rsi, &tmpBuffer2);
    if (err != ERR_NONE)
    {
      return err;
    }

    err = readStringFromProcessMemory(executableHandle->pid, executableHandle->regs.rdx, &tmpBuffer3);
    if (err != ERR_NONE)
    {
      return err;
    }
    ...
    printf("mount(dev-name=\"%s\", dir-name=\"%s\", type=\"%s\", flags=0x%08x, data=\"%s\")\n",
                  tmpBuffer1,
                  tmpBuffer2,
                  tmpBuffer3,
                  data);
...
/***********************************************************************************/
  ...
  case SYS_io_getevents:
    ...
    err = readProcessMemoryFromPID(executableHandle->pid, executableHandle->regs.r8, &timeSpec, sizeof(struct timespec));
    if (err != ERR_NONE)
    {
      return err;
    }
  ...
  case SYS_clock_settime:
    ...
    printf("clock_settime(timerID=%d, seconds=%d.%d)\n",
                          executableHandle->regs.rdi,
                          timeSpec.tv_sec,
                          timeSpec.tv_nsec);
  ...
  case SYS_clock_gettime:
    ...
    printf("clock_gettime(timerID=%d, seconds=%d.%d)\n",
                          executableHandle->regs.rdi,
                          timeSpec.tv_sec,
                          timeSpec.tv_nsec);
    printf("Returned With: %d\n\n", executableHandle->regs.rax);
...
/***********************************************************************************/
  ...
  case SYS_clock_nanosleep:
    ...
    printf("clock_nanosleep(timerID=%d, seconds=%d.%d)\n",
                            executableHandle->regs.rdi,
                            timeSpec.tv_sec,
                            timeSpec.tv_nsec,
...
/***********************************************************************************/
  case SYS_preadv:
    int iovCnt = executableHandle->regs.rdx;

    PROGRESS_TO_SYSCALL_EXIT(executableHandle->pid);

    printf("preadv(fd=%d, iov-base-addr=%p, iovcnt=%d, offset=%p)\n",
                   executableHandle->regs.rdi,
                   executableHandle->regs.rsi,
                   executableHandle->regs.rdx,
                   executableHandle->regs.r10);
    ...
    if (iovCnt > 0)
    {
      tmpBuffer1 = malloc(sizeof(struct iovec) * iovCnt);
      if (tmpBuffer1 == NULL)
      {
        return ERR_MEMORY_ALLOCATION_FAILED;
      }
      ...
      for (int i = 0; i < iovCnt; i++)
      {
        ...
        puts("Data:\n\n");
        err = dumpHexBytesFromOffset(tmpBuffer2, tmpBuffer2, vec->iov_len);
    ...
    for (int i = 0; i < iovCnt; i++)
    {
      ...
      {
        vec = (struct iovec *) (tmpBuffer1 + sizeof(struct iovec) * i);
        // This code is incorrect, we could use dumpHexBytesFromOffset function.
        // printf("%02x, ", vec[j]);
        // if (j == vec[0].iov_len)
        // {
...
/***********************************************************************************/
  case SYS_setns:
    ...
    // The following function also handles printing the syscall.
...
/***********************************************************************************/
  case SYS_getcpu:
    int cpu = 0;
    int node = 0;
    // Progress execution to the return point of getcpu()
    PROGRESS_TO_SYSCALL_EXIT(executableHandle->pid);

    if (executableHandle->regs.rdi != 0)
    {
      err = readProcessMemoryFromPID(executableHandle->pid,
                                     executableHandle->regs.rdi,
                                     &cpu,
...
/***********************************************************************************/
  case SYS_process_vm_readv:
    unsigned long riovcnt = executableHandle->regs.r8;

    if (riovcnt > 0)
    {
      // Get the data from remote vector.
      tmpBuffer1 = malloc(sizeof(struct iovec) * riovcnt);
      if (tmpBuffer1 == NULL)
      {
        return ERR_MEMORY_ALLOCATION_FAILED;
      }

      err = readProcessMemoryFromPID(executableHandle->regs.rdi, // We are reading from the remote process here.
                                     executableHandle->regs.r10,
                                     (struct iovec *)&tmpBuffer1,
                                     sizeof(struct iovec));
      if (err != ERR_NONE)
      {
        return err;
    ...
    {
      struct iovec tmpIoVec = {0};
      memcpy(&tmpIoVec, (tmpBuffer1 + sizeof(struct iovec) * i), sizeof(struct iovec));

      printf("iovec.iov_base=0x%016x\niovec.iov_len=0x%016x\n\n",
             tmpIoVec.iov_base,
             tmpIoVec.iov_len);
    }
    ...
    PROGRESS_TO_SYSCALL_EXIT(executableHandle->pid);
    printf("Returned With: %d\n\n", executableHandle->regs.rax);
    break; /*SYS_process_vm_readv*/

/***********************************************************************************/
  case SYS_process_vm_writev:
    unsigned long liovcnt = executableHandle->regs.rdx;

    if (liovcnt > 0)
    {
      // Get the data from remote vector.
      tmpBuffer1 = malloc(sizeof(struct iovec) * liovcnt);
      if (tmpBuffer1 == NULL)
      {
        return ERR_MEMORY_ALLOCATION_FAILED;
      }
...
/***********************************************************************************/
  case SYS_membarrier:
    ...
    tmpBuffer1 = malloc(47); // Length of longest macro string + 1
    if(tmpBuffer1 == NULL)
    {
      return ERR_MEMORY_ALLOCATION_FAILED;
    }

    memset(tmpBuffer1, 0, 47);

    switch(executableHandle->regs.rdi) // membarrier 'cmd' argument
    {
      case MEMBARRIER_CMD_QUERY:
        strncpy(tmpBuffer1, "MEMBARRIER_CMD_QUERY", 20);
        break;

      case MEMBARRIER_CMD_GLOBAL:
        strncpy(tmpBuffer1, "MEMBARRIER_CMD_GLOBAL", 21);
        break;

      case MEMBARRIER_CMD_GLOBAL_EXPEDITED:
        strncpy(tmpBuffer1, "MEMBARRIER_CMD_GLOBAL_EXPEDITED", 31);
        break;

      case MEMBARRIER_CMD_REGISTER_GLOBAL_EXPEDITED:
        strncpy(tmpBuffer1, "MEMBARRIER_CMD_REGISTER_GLOBAL_EXPEDITED", 40);
        break;

      case MEMBARRIER_CMD_PRIVATE_EXPEDITED:
        strncpy(tmpBuffer1, "MEMBARRIER_CMD_PRIVATE_EXPEDITED", 32);
        break;

      case MEMBARRIER_CMD_REGISTER_PRIVATE_EXPEDITED:
        strncpy(tmpBuffer1, "MEMBARRIER_CMD_REGISTER_PRIVATE_EXPEDITED", 41);
        break;

      case MEMBARRIER_CMD_PRIVATE_EXPEDITED_SYNC_CORE:
        strncpy(tmpBuffer1, "MEMBARRIER_CMD_PRIVATE_EXPEDITED_SYNC_CORE", 42);
        break;

      case MEMBARRIER_CMD_PRIVATE_EXPEDITED_RSEQ:
        strncpy(tmpBuffer1, "MEMBARRIER_CMD_PRIVATE_EXPEDITED_RSEQ", 37);
        break;

      case MEMBARRIER_CMD_REGISTER_PRIVATE_EXPEDITED_RSEQ:
        strncpy(tmpBuffer1, "MEMBARRIER_CMD_REGISTER_PRIVATE_EXPEDITED_RSEQ", 46);
        break;

      // These two constants cause a compilation error.
      // TODO: Can this be fixed?

      // case MEMBARRIER_CMD_SHARED:
      //   break;

      // case MEMBARRIER_CMD_FLAG_CPU:
      //   break;
    }
...
/***********************************************************************************/
  case SYS_copy_file_range:
     ...
     * all bytes with all their bits set.
    */
    dumpHexBytesFromOffset(tmpBuffer1, 0, executableHandle->regs.r8);

    printf("Returned With: %d\n\n", executableHandle->regs.rax);
    break; /*SYS_copy_file_range*/
...
/***********************************************************************************/
  case SYS_move_mount:
    ...
    if(err != ERR_NONE && err != ERR_NULL_VALUE_READ_FROM_MEMORY && err != ERR_MEMORY_ALLOCATION_FAILED)
    {
      free(tmpBuffer1);
    }
    if(err == ERR_NULL_VALUE_READ_FROM_MEMORY)
...
/***********************************************************************************/
  case SYS_fspick:
    ...
    tmpBuffer1 = readStringFromProcessMemory(executableHandle->pid,
                                             executableHandle->regs.rsi,
                                             &tmpBuffer1);
    if(err != ERR_MEMORY_ALLOCATION_FAILED)
    {
      free(tmpBuffer1);
    }
    if(err != ERR_NONE && err != ERR_NULL_VALUE_READ_FROM_MEMORY)
...
/***********************************************************************************/
  case SYS_close_range:
    printf("close_range(fd=%d, max-fd=%d, flags=0x%08x)\n",
                        executableHandle->regs.rdi,
                        executableHandle->regs.rsi,
                        executableHandle->regs.rdx);
    ...
    PROGRESS_TO_SYSCALL_EXIT(executableHandle->pid);
    printf("Returned With: %d\n\n", executableHandle->regs.rax);
    break; /*SYS_close_range*/

/***********************************************************************************/
  case SYS_openat2:
    struct open_how openHow = {0};
    ...
    printf("openat2(dfd=%d, pathname=\"%s\", flags=0x%08x)\n",
                    executableHandle->regs.rdi,
                    executableHandle->regs.rsi,
                    executableHandle->regs.rdx);
...
/***********************************************************************************/
  case SYS_faccessat2:
    ...
    printf("faccessat2(fd=%d, fd=%d, flags=0x%08x)\n",
                       executableHandle->regs.rdi,
                       executableHandle->regs.rsi,
                       executableHandle->regs.rdx);
    ...
    PROGRESS_TO_SYSCALL_EXIT(executableHandle->pid);
    printf("Returned With: %d\n\n", executableHandle->regs.rax);
    break; /*SYS_faccessat2*/

/***********************************************************************************/
  case SYS_process_madvise:
    printf("process_madvise(pidfd=%d, max-fd=%d, flags=0x%08x)\n",
                        executableHandle->regs.rdi,
                        executableHandle->regs.rsi,
                        executableHandle->regs.rdx);
...
/***********************************************************************************/
  case SYS_process_mrelease:
    printf("process_mrelease(pidfd=%d, flags=0x%08x)\n",
                             executableHandle->regs.rdi,
                             executableHandle->regs.rsi);

    PROGRESS_TO_SYSCALL_EXIT(executableHandle->pid);
    printf("Returned With: %d\n\n", executableHandle->regs.rax);
    break; /*SYS_process_mrelease*/

/***********************************************************************************/
  case SYS_futex_waitv:
    struct futex_waitv futexWaitv = {0};

    err = readProcessMemoryFromPID(executableHandle->pid,
                                   executableHandle->regs.rdi,
                                   &futexWaitv,
                                   sizeof(struct futex_waitv));
    if(err != ERR_NONE)
    {
      return err;
    }
    ...
    printf("futex_waitv(futex_waitv-val=%d, futex_waitv-uaddr=0x%016x, " \
                       "futex_waitv-flags=0x%08x, nr_futexes=%d, flags=0x%08x, " \
                       "timeout=%d.%d, clockID=%d)\n",
                        futexWaitv.val,
                        futexWaitv.uaddr,
                        futexWaitv.flags,
                        executableHandle->regs.rsi,
    ...
    PROGRESS_TO_SYSCALL_EXIT(executableHandle->pid);
    printf("Returned With: %d\n\n", executableHandle->regs.rax);
    break; /*SYS_futex_waitv*/

/***********************************************************************************/
  case SYS_set_mempolicy_home_node:
    printf("SYS_set_mempolicy_home_node(start=0x%016x, len=0x%08x, home-node=0x%016x, flags=0x%016x)\n",
                                        executableHandle->regs.rdi,
                                        executableHandle->regs.rsi,
                                        executableHandle->regs.rdx,
                                        executableHandle->regs.r10);
    ...
    PROGRESS_TO_SYSCALL_EXIT(executableHandle->pid);
    printf("Returned With: %d\n\n", executableHandle->regs.rax);
    break; /*SYS_set_mempolicy_home_node*/

/***********************************************************************************/
  case __NR_cachestat:
    struct cachestat_range cacheRange = {0};
    struct cachestat cacheStat = {0};
    ...
    printf("SYS___NR_cachestat(fd=%d, cstat_range-off=0x%016x, cstat_range-len=0x%016x, cstat-cache=0x%016x, " \
                              "cstat-dirty=0x%016x, cstat-writeback=0x%016x, cstat-evicted=0x%016x, " \
                              "cstat-recently_evicted=0x%016x, flags=0x%08x)\n",
                               executableHandle->regs.rdi,
                               cacheRange.off,
                               cacheRange.len,
                               cacheStat.nr_cache,
                               cacheStat.nr_dirty,
                               cacheStat.nr_writeback,
                               cacheStat.nr_evicted,
                               cacheStat.nr_recently_evicted,
                               executableHandle->regs.r10);

    printf("Returned With: %d\n\n", executableHandle->regs.rax);
    break; /*SYS___NR_cachestat*/
...
static void unittest_mapELF32ToHandleFromProcessMemory_legalEhdr()
{
  /*
   * Typical Elf32_Ehdr
   */
...
static void unittest_mapELF32ToHandleFromProcessMemory_legalEhdr_differentValues()
{
  /*
...
static void unittest_mapELF32ToHandleFromProcessMemory_nullMemoryPtr()
{
  char *ptr = NULL;
  ELF32_EXECUTABLE_HANDLE_T *handle = NULL;
  int8_t err = ERR_NONE;
...
static void unittest_mapELF64ToHandleFromProcessMemory_legalEhdr()
{
  /*
   * Typical Elf64_Ehdr
   */
...
static void unittest_mapELF64ToHandleFromProcessMemory_legalEhdr_differentValues()
{
  /*
...
static void unittest_mapELF64ToHandleFromProcessMemory_nullMemoryPtr()
{
  char *ptr = NULL;
  ELF64_EXECUTABLE_HANDLE_T *handle = NULL;
  int8_t err = ERR_NONE;
...
static void unittest_printNamespaceType()
{
  char buffer[100];
  int pipefd[2];
  ...
  // What used to be sent to stdout will now go to the pipe.
...
static void unittest_print_kcmpType()
{
  assert(print_kcmpType(KCMP_FILE) == KCMP_FILE);
  puts("\n");
  assert(print_kcmpType(KCMP_FILES) == KCMP_FILES);
  puts("\n");
  assert(print_kcmpType(KCMP_FS) == KCMP_FS);
  puts("\n");
  assert(print_kcmpType(KCMP_IO) == KCMP_IO);
  puts("\n");
  assert(print_kcmpType(KCMP_SIGHAND) == KCMP_SIGHAND);
  puts("\n");
  assert(print_kcmpType(KCMP_SYSVSEM) == KCMP_SYSVSEM);
  puts("\n");
  assert(print_kcmpType(KCMP_VM) == KCMP_VM);
  puts("\n");
  assert(print_kcmpType(KCMP_EPOLL_TFD) == KCMP_EPOLL_TFD);
  puts("\n");

  // Test some values that are not contained in the kcmp_type enum.
  assert(print_kcmpType(100) == ERR_UNKNOWN_EXPECTED_VALUE);
  puts("\n");
  assert(print_kcmpType(15) == ERR_UNKNOWN_EXPECTED_VALUE);
  puts("\n");
  assert(print_kcmpType(-1) == ERR_UNKNOWN_EXPECTED_VALUE);
  puts("\n");
  assert(print_kcmpType(10000000000) == ERR_UNKNOWN_EXPECTED_VALUE);
  puts("\n");
}
